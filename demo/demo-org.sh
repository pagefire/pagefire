...
# --- Create users ------------------------------------------------------------
AAA_ID=$(curl -sf -X POST "${API}/users" \
  -H "$(auth)" -H "Content-Type: application/json" \
  -d '{"name":"User AAA","email":"aaa@xyz.dev","timezone":"America/Los_Angeles"}' | jq_id)
ok "Created User AAA -- Senior SRE"
...
ONCALL_NAME=$(curl -sf "${API}/oncall/${SCHEDULE_ID}" \
  -H "$(auth)" | python3 -c "import sys,json; users=json.load(sys.stdin); print(users[0]['name'] if users else 'nobody')")
...
echo "  Alerts: curl -s -H 'Authorization: Bearer ${ADMIN_TOKEN}' \\"
